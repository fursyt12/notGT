...
`x`/`y` (в процентах от сцены out'а), `scale`, `enabled`, `order` и **своя** `playback`,
которая переопределяет шаблонную:
...
- Replicant `runtime` — производное: `playing` (`{outId: [itemId, ...]}`) и `revision`.
  Пересчитывается на сервере и не сохраняется.
...
### notGT — Control (`control.html`)

Операторская панель «здесь и сейчас»:

- редактирование глобальных **переменных** (data) — то, что подставляется в `{{...}}`;
- **show / hide** активного титра (в т.ч. с выбором out'а и label);
- **trigger** — одиночный проигрыш анимации;
- быстрый просмотр текущего состояния (`active`, `playing`, `revision`).

Это панель для эфира: она не меняет конфигурацию, только гоняет программу.
...
Полноэкранный редактор одной анимации:

- визуальный canvas-редактор на **Konva** — слои перетаскиваются, масштабируются,
  выстраиваются по `z`; свойства (шрифт, цвет, обводка, тень и т.д.) правятся тут же;
- **редактор кода** для шаблонов `kind: "code"` (HTML / CSS / JS).
